...
\begin{figure}[p] \vspace*{-3cm} \centering
  \noindent\includegraphics[width=\textwidth]{figures/chapter-seasonal/zm_winds_sh_poststamp.pdf}
...
We have repeated the analysis of SAM and SOI correlations presented in the
review, using data from BAS (SAM) and BOM (SOI). For the 1996-2009 hindcast
period we find the correlation of June-July SOI with Oct-Nov SAM to be $r=0.63$
which is broadly in agreement with the results stated in the review (the slight
difference may be due to different data sources). For 1979-2013 (the ERA-Interim
period) $r=0.32$ (and for the full 1957-2013 $r=0.01$). In order to justify
using the shorter hindcast period (with higher correlation) in Fig. 7(c), it
would need to be the case that the SAM/SOI correlation is statistically
significantly stronger during the hindcast period than the ERA-Interim period,
so that these different correlations are not a result of random variability.
...
calculates the correlation. Figure \ref{fig:SAM_SOI} (of this document) shows a
histogram of these correlations along with the 1996-2009 correlation. The
...
the longer ERA-Interim period in our analysis to provide a more robust measure of
sources of predictability. For completeness we include a figure with the same
analysis limited to 1996--2009 in this document
(Fig. \ref{fig:lag_height}). This is not shown in the paper but discussed in
section 3(d).  
