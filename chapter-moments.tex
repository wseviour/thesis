...
clusters; \citet{K.Coughlin2009} found two clusters using a silhuette values
...
sensistivity tests were performed by randomly removing 1\% of the data and
...
model simulations. The liklely reason for this sensitivity is that the data
...
method below) as CP07 and M13. \citet{Mitchell2011} found that the aspect ratio
and centroid latitude follow an extreme value distribution \citep{Cole} and it
is noted that both thresholds chosen here lie beyond the extreme value
thresholds of their respective distributions. 
...
average of 7 per decade. This frequency lies between the values of CP07
(6/decade) and M13 (8/decade). Although data is restricted to DJFM in this
...
\ref{fig:z_m13_cp07_histogram}). Some events, particularly those occuring in
...
the longer period studied here because of the difficulties with heirarchical
...
averages 5.7~K [3.0, 8.3] for split and 6.8 [5.5, 8.2] for displaced vortex
...
events, both the current method and M13 show a skew towards events occuring
...
\begin{figure}
 ...
 \caption[NAM composites for split and displaced vortex events.]{Composites of
   ...
   and (b) 18 splitting events. (c) shows the difference in thesese composites,
   ...
   unshaded. Data is from the ECMWF Reanalyses 1958-2009. COLOURBAR SIGN!!!}
...
events greatly differs, with split vortex events occuring almost instantaneously
...
vortex events suggests that resonant excitation of the brotropic normal mode
\citep{Esler2005} is an important influence in this case. Furthermore, the rapid
onset of split vortex events is ... (Love 1893).
...
are not. They did not, however, test the \emph{difference} betwen split and
...
calcilated from this distribution. 

The strongest precursor is found for displaced vortex events, with a wave-1
pattern that is similar to the cimatological stationary wave pattern
...
more negative MSLP anomaly over Sacandinavia and Siberia following displaced
...
It is possible to quantitively examine (although only approximately) whether
these tropopasue anomalies are consistent with changes in stratospheric PV
...
displaced stratospheric polar vortex events because of their different dynamical
evolution and impacts on the troposphere. However, previous methods to identify
these events are impractical for application to climate model simulations
because they are highly sensitive to model climatology or rely on non-standard
variables. In this chapter a new method has been developed to identify displaced
and split vortex events, which can be easily applied to climate model
simulations because it requires only geopotential height at 10~hPa. The method
...
\item To identify the vortex region, a single contour of 10~hPa geopotential
  ...
  $60^{\circ}$N.
\item Using this contour and the geopotenial height field, the centroid latitude
  and aspect ratio moment diagnostics are calculated.
...
demonstrates that it is able to identify split and displaced vortex events at
least as effectively as previous methods.
